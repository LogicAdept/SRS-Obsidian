...
`markSupported()` is `false`. `mark` is a no-op; `reset` throws `IOException`. After `close()`, `read` / `unread` / `available` / `skip` throw `IOException`. Closing the pushback stream **closes the underlying** `in`. This is a **byte** stream; character pushback is `PushbackReader` ([[What is the difference between InputStream OutputStream Reader and Writer]]).
