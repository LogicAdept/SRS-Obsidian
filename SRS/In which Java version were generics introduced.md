...
Implementation is **type erasure**: parameters exist at compile time and are erased afterward. There is no new class per `List<String>` vs `List<Integer>`. You cannot ask the VM for `T` at run time the way C++ templates specialize. That is the interop story with legacy collections — and the reason `Collections.checkedSet` (and friends) exist if you must catch a bad insert at the boundary ([[Why does type erasure prevent a List of int in Java]], [[How does type erasure work for Java generics]]).
