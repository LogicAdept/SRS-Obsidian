...
**`RANDOM_PORT` / `DEFINED_PORT`** start a real server (`spring-boot-web-server`); inject the port with **`@LocalServerPort`**. **`DEFINED_PORT`** binds **`server.port`** from config or **8080** (port fights in parallel CI). **`NONE`** sets **`WebApplicationType.NONE`**. HTTP clients: **`@AutoConfigureRestTestClient` / `@AutoConfigureTestRestTemplate`**. Relative URIs on those clients already point at the running server. MVC in-process: **`@AutoConfigureMockMvc`** (not implied by `@SpringBootTest` alone).

A **slice** (`@WebMvcTest`, `@DataJpaTest`) is a **different** `@…Test`: restricted scan and auto-config. Dump “unit-speed `@SpringBootTest` + `@MockBean`” is **`@MockitoBean`** on Boot **4**, and a slice is usually the right tool. The TestContext **cache** reuses the same configuration key. Annotation family: [[What are Spring Boot test slices]]. Modes: [[What is SpringBootTest WebEnvironment]]. Live HTTP: [[How do you test REST endpoints end to end]]. MockMvc: [[How do you write a MockMvc test]]. MVC slice: [[What is the WebMvcTest annotation]]. JPA slice: [[What is the DataJpaTest annotation]].
...
> **`MOCK`**. `@LocalServerPort` and live **`TestRestTemplate`** need **`RANDOM_PORT` or `DEFINED_PORT`** plus the matching **`@AutoConfigure…`**. On a real port the server runs on a **different thread** than the test, so method-level **`@Transactional` rollback does not undo server-side work**.
