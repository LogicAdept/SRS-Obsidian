...
**Compilation.** `javac` (or another compiler) emits a machine-independent `class` file. A JVM **loads, links, initializes**, and executes that file — optionally compiling further to native code. The same binaries run wherever a compatible JVM exists ([[How does the JVM help Java bytecode run across operating systems]], [[Why is Java described as platform independent]]). Tools vs runtime: [[What is the difference between the JDK and the JRE]]. Entry: [[How would you explain the Java main method entry point]].
