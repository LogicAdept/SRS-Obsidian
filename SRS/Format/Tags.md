...
**Serialization in the tree:** **`#Java/Serialization`** is Java object serialization (what/why, the default `Serializable` field protocol, the binary stream, `writeObject`/`readObject` catalogs, format design that spans named fields and Externalizable). **SerialVersionUID** is the stream class stamp (`serialVersionUID`, when to declare or change it, default hash inputs). **SingletonSerializationProblem** is deserialize allocating a second singleton (`readResolve`, enum singleton). **Externalizable** is the custom-payload protocol (`writeExternal` / `readExternal`, public no-arg constructor). **NotSerializable** is refusing the protocol (omit the marker, throw `NotSerializableException`). **Transient** is excluding instance serial fields (`transient`, omitting a name from `serialPersistentFields`). Dual-tag **`#Java/Language/Modifiers`** on `transient` keyword cards. **Validation** is checking restored state (`readObject` checks, `ObjectInputValidation`, `SealedObject` / `SignedObject`). Dual-tag **`#Java/Security`** when the cue is cryptographic wrappers. Record / enum / Optional serialization dual-tags those language leaves. Do not put **`#Java/Serialization`** next to a more specific **`#Java/Serialization/...`** child on the same card.

**JPA in the tree:** **`#Java/Persistence/JPA`** is Jakarta Persistence as a specification (what JPA is, spec vs Hibernate, `EntityManager` lifecycle, JPQL/Criteria, locking). **Mapping** is entity/table/column/id metadata, entity-class rules, and association/`CascadeType` mapping. **Fetching** is `FetchType`, lazy/eager, `join fetch`, entity graphs, and lazy-loading costs. Dual-tag **`#Java/Persistence/Hibernate`** on provider-native APIs, **`#Java/Spring/Data/JPA`** on Spring Data JPA repositories, and **`#Java/Annotations`** on JPA annotation cards. Do not put **`#Java/Persistence/JPA`** next to a more specific **`#Java/Persistence/JPA/...`** child, or **`#Java/Persistence`** next to **`#Java/Persistence/JPA`**, on the same card.
...
**Concurrency in the tree:** **`#Java/Concurrency`** is the Java threading model. Child leaves: **`Threads`** (lifecycle, `Thread`/`Runnable`, join/sleep/interrupt/daemon), **`Executors`** (pools, `Future`/`Callable`/`CompletableFuture`, ForkJoin), **`Synchronizers`** (`CountDownLatch`, `CyclicBarrier`, `Phaser`, `Semaphore`, `Exchanger`), **`Atomics`** (`java.util.concurrent.atomic`, CAS), **`VirtualThreads`**, **`Synchronization`** (monitors, wait/notify, intrinsic locks). **`Synchronization/SynchronizedKeyword`** is the `synchronized` keyword (methods, blocks, `this` vs `Class` monitor, private mutex objects). **`Synchronization/Locks`** is `java.util.concurrent.locks` (`Lock`, `ReentrantLock`, `ReadWriteLock`, `StampedLock`, `Condition`). Concurrent collections stay **`#Java/Collections/Concurrency`**. **`CopyOnWrite`** (`CopyOnWriteArrayList` / `CopyOnWriteArraySet`, snapshot iterators, write-copy cost) is **`#Java/Collections/Concurrency/CopyOnWrite`**. The Java Memory Model stays **`#Java/JMM`**. **`JMM/HappensBefore`** is the happens-before relation itself: the synchronizes-with actions and the JLS 17.4.5 rules (program order, monitor lock, volatile, thread start, thread termination, interrupt, default value, finalizer, transitivity) plus class initialization (JLS 12.4.2), the DRF-SC guarantee and the data-race allowed-reads regime, and the j.u.c memory consistency properties (concurrent collections, executor submission, `Future.get`, synchronizer release/acquire, barriers, `Exchanger`, atomics, VarHandle access modes). Final field semantics (JLS 17.5), visibility / ordering overviews, and non-atomicity pitfalls stay on **`#Java/JMM`**. Do not put **`#Java/Concurrency`** next to a more specific **`#Java/Concurrency/...`** child, **`#Java/Concurrency/Synchronization`** next to a more specific **`#Java/Concurrency/Synchronization/...`** child, **`#Java/Collections/Concurrency`** next to **`#Java/Collections/Concurrency/CopyOnWrite`**, or **`#Java/JMM`** next to **`#Java/JMM/HappensBefore`**, on the same card.
...
**Streams in the tree:** **`#Java/Streams`** is the `java.util.stream` API (what a `Stream` is, vs `Collection`, sources/creation, pipeline internals / `Spliterator`, `Optional.stream()`). **Operations** is the two kinds (intermediate vs terminal) as a catalog, laziness, and when the pipeline starts. **Operations/Intermediate** is intermediate ops (`filter`, `map` / `flatMap`, `sorted`, `limit`, `distinct`, `peek`). **Operations/Terminal** is terminal ops (`collect` / `Collector`, `forEach` / `forEachOrdered`, `reduce`, `min` / `max` / `sum` / `average`, `count`). **Parallel** is `parallelStream()` / `parallel()`, sequential vs parallel, and ForkJoin / `Spliterator` backing (dual-tag **`#Java/Parallelism`**). Sequential vs parallel is a mode, not a third kind of operation. Byte/char IO streams stay **`#Java/IO/Streams`** (the **`#Java/IO`** parent is for IO-vs-NIO comparisons and close/`IOException` catalogs). Reactive streams stay **`#Java/Library/Reactor`** / **`#Paradigms/Reactive`**. Do not put **`#Java/Streams`** next to a more specific **`#Java/Streams/...`** child, or **`#Java/Streams/Operations`** next to a more specific **`#Java/Streams/Operations/...`** child, on the same card.

**Lambdas in the tree:** **`#Java/Lambdas`** is lambda expressions (syntax, target typing, capture, body size, checked exceptions from the body). **`#Java/MethodReferences`** is `::` (kinds, `invokedynamic`); dual-tag Lambdas. **`#Java/FunctionalInterfaces`** is the `java.util.function` SAM catalog; dual-tag Lambdas when the cue is a lambda target. Capture / effectively final dual-tags **`#Java/Language/Modifiers/Final`**. Nested vs lambda dual-tags **`#Java/Language/NestedClasses`**. Comparator lambdas dual-tag **`#Java/Collections/Sorting/Comparator`**. Do not use Lambdas alone for a method-reference identity card.
...
**String in the tree:** **`#Java/String`** is `java.lang.String` (immutability, intern API, compact strings, `toCharArray`, literals as the type). **StringBuffer** is `java.lang.StringBuffer` (synchronized mutable buffer, Java 1.0). **StringBuilder** is `java.lang.StringBuilder` (unsynchronized drop-in, Java 5). **StringJoiner** is `java.util.StringJoiner`. Comparison cards carry both sides. Pool *location* stays **`#Java/JVM/Memory`** (Heap for interned `String` objects). Dual-tag **`#Java/String`** when the cue is the string type or intern API rather than the pool as a region. Do not put **`#Java/String`** next to a more specific **`#Java/String/...`** child on the same card.
...
* `#Java/Serialization/NotSerializable`
* `#Java/Serialization/Transient`
* `#Java/Serialization/Validation`
* `#Java/Generics`
