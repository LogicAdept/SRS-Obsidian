...
Three rules turn "a cluster of objects" into an aggregate. First, identity from outside: other aggregates hold the root's ID (`OrderId`), not a reference to the whole graph, which keeps aggregate graphs small and loadable. Second, one transaction per aggregate: commit changes to `Order` or to `Invoice`, never both in one transaction; everything else propagates via domain events eventually (see [[How does an aggregate persist and publish events without a distributed transaction]]). Third, small boundaries: an aggregate spanning dozens of objects becomes a serialization bottleneck - every writer contends on the root. Vernon's implementation guidance and Microsoft's DDD docs both state the one-aggregate-per-transaction rule as the practical core.

> [!warning] "Aggregate = parent table with child tables" is the lazy reading
> The aggregate is a consistency boundary, not a database join shape. What makes it an aggregate is that the root can refuse illegal states and outside code cannot bypass it - not that the tables share a foreign key. Teams that skip the root's guarding role end up with "aggregates" whose invariants are enforced, if at all, in service code - which is just [[What is an anemic domain model and is it useful|an anemic model]] with extra nouns. The same confusion usually comes from never separating entities from [[What is a value object and why should you use one|value objects]] in the first place.

> [!tip] Interview answer
> Entity: identity-defined and stateful over time. Value object: immutable, identity-free, compared by value. Aggregate: a cluster that changes as one consistent unit; the root is the only entry point and enforces invariants across it. Two rules I always mention: reference other aggregates by ID, and one transaction touches one aggregate - cross-aggregate effects go through domain events.
