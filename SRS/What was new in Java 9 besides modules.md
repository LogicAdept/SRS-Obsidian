...
> "G1 became default in 9" is correct; claiming CMS was removed in 9 is not — CMS was **deprecated** in 9 (JEP 291) and **removed in 14** (JEP 363). The HTTP client in 9 is an **incubator** module — code against `jdk.incubator.http` does not compile on 11, where it moved to `java.net.http` ([[What is an incubator module]]). And `Set.of` iteration order is unspecified — `[3, 2, 1]` in the listing is not a contract ([[What does Set.of return and what happens with duplicates]]). The full collector lifecycle lives in [[Which garbage collectors were added or removed in each Java version]].
