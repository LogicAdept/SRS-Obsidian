...
Normalization works by decomposition: given a table that mixes facts about different entities, split it so each relation holds facts about one entity keyed by one key, and connect the pieces with foreign keys. The normal forms grade the result. **1NF** — atomic values, no repeating groups; **2NF** — no non-key fact depending on only part of a composite key; **3NF** — no non-key fact depending on another non-key field. The usual mnemonic for 2NF/3NF is that every field must provide a fact about "the key, the whole key, and nothing but the key". Further forms tighten the edge cases: BCNF requires every determinant to be a key ([[What is Boyce-Codd normal form and how does it differ from 3NF]]), 4NF splits independent multivalued facts ([[What is fourth normal form and what are multivalued dependencies]]), 5NF handles three-way join dependencies ([[What is fifth normal form and when does a three-way decomposition matter]]), and 6NF decomposes per attribute for temporal timelines ([[What is sixth normal form and where does per-attribute decomposition pay off]]); OLTP schemas are normally engineered to 3NF, which removes the anomalies that matter in day-to-day writes.
...
The primitive the ladder reasons with is the functional dependency: a rule stating that one set of columns determines another — every normal form above 1NF is a progressively stricter statement about which dependencies may remain. The relational model the whole ladder rests on is drilled in [[How would you explain the relational data model and Codd rules basics]].
