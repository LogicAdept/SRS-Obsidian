...
> The centralized exception tracking pattern collects every exception from every service into one system that deduplicates, groups and counts them — so each distinct problem becomes one tracked item with a stack trace, metadata and a trend, rather than a needle in a thousand log lines. An observability pattern; the canonical category is Sentry-class tools (Raven/DSN clients), distinct from log aggregation in purpose and data model.
