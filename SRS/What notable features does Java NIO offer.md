...
What each cluster buys you. **Buffers**: direct buffers (`allocateDirect`) place data outside the heap so OS calls can use it without a copy; `flip`/`clear`/`rewind` control the read-write switch. Every buffer obeys `0 ≤ mark ≤ position ≤ limit ≤ capacity`; there is **no** boolean buffer, and buffers are **not** thread-safe ([[What is a Buffer in Java NIO and how do position limit and capacity work]], [[What is the difference between direct and heap ByteBuffers]]). **Channels**: one abstraction for files and sockets with bulk moves (`transferTo`/`transferFrom`, which can become zero-copy OS operations), memory-mapped files (`FileChannel.map`) that turn a file into a byte region addressable in memory, and file locks (`FileChannel.lock`). **Selectors**: readiness-based multiplexing — see [[How does NIO provide non-blocking access to resources]]. A channel in blocking mode refuses to register (`IllegalBlockingModeException`); you opt in with `configureBlocking(false)` first. **NIO.2**: `Path`/`Files` replacing `File` with attribute views, recursive `walkFileTree`, symlink handling, and a `WatchService` for directory change events; plus the asynchronous channel group model for completion-based I/O ([[What is asynchronous IO in Java NIO]]). **Charsets**: `java.nio.charset` sells named encoder/decoder objects between characters and bytes — the bridge every reader and writer uses ([[How does Java handle character encoding when reading and writing files]]).
