...
> The API gateway is the single entry point all clients hit instead of calling services directly: it routes each request to the owning service and often enforces the cross-cutting edge concerns once — authentication, rate limiting, TLS termination, response shaping. It is the microservices equivalent of the Facade pattern; behind it, services may change address, split or merge without client releases.
