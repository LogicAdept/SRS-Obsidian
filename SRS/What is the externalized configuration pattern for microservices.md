...
> Externalized configuration moves everything that varies between environments — database locations, credentials, feature endpoints, queue addresses — out of the deployable and into the environment: the service reads configuration at startup from sources like OS environment variables, config files or a config server. It is the 12-factor "config" factor and Richardson's cross-cutting pattern; the artifact must run in dev, QA and production unmodified.
