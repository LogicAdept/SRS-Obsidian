...
The Docker fill orchestrator rebuilds the index once after fill turns on
`--until-tag` (or `--finalize`), after `/refine-tags` and `/dedup-tag`. Do not
rebuild during a Docker fill turn.
