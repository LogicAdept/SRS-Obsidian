...
def render_index(
    *,
    ...
    lines = [
        ...
        "shape misses such as a missing warning get an automatic repair send). "
...
<header>
  <details class="hints">
    ...
    <dt>Fill agent</dt>
    <dd><b>Start</b> — изолированный fill на одном Cursor-агенте. Сначала модель группирует оставшиеся <code>#New</code> по общему механизму и официальным докам. Один ход = один такой кластер; если после fill нет warning или остался URL, оркестратор сам шлёт repair, затем следующий кластер, пока под тегом не кончатся <code>#New</code>. В shell нужен <code>CURSOR_API_KEY</code>.</dd>
    <dt>Cover clone</dt>
